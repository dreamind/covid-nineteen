...
export default new Vuex.Store({
  ...
  actions: {
    async pull(context) {
      let table = await api.pull();
      context.commit("setTable", table);
    },
    getSnapshot(context, currentDate, currentDateIdx) {
      let { state } = context;
      if (!currentDate) {
      ...
      context.commit("setSnapshot", { currentDate, currentDateIdx, rows, rowStats });
    },
    updateCountries(context, country) {
      let { currentCountries, rowMap } = context.state;
      let newCountries;
      let selected;
